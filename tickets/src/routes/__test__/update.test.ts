...
it("returns a 401 if the user is not authenticated", async () => {
  ...
  await request(app).put(`/api/tickets/${id}`).send({ title: "A new title", price: 10 }).expect(401);
});
...
it("updates the ticket if provided valid inputs", async () => {
  ...
  await request(app).put(`/api/tickets/${response.body.id}`).set("Cookie", cookie).send(body).expect(200);
...
it("rejects edits if the ticket is reserved", async () => {
  ...
  await request(app).put(`/api/tickets/${response.body.id}`).set("Cookie", cookie).send(body).expect(400);
});
